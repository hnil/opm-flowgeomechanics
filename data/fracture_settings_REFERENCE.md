...
| `leakoff_mobility` | which mobility λ multiplies kA/L: `reservoir_total` (default, the original — every active phase mobility of the reservoir cell, summed) or `upwind` (what the flow's water equation applies across the same connection — the fracture cell's water mobility where the fracture feeds the formation, the reservoir cell's water mobility where the flow reverses). Equal on a single-phase deck, a factor 2 to 3 apart on model2. Affects the fracture's own pressure solve and, through it, the upscaled well indices; the embedded connection is unchanged either way, since the aux path divides back out the mobility that was folded in. |
| `embedded_well_source` | embedded flow only: which fracture cells the well feeds. `ring` (default) = the six seed-ring cells around the wellbore, whatever `well_source_all_perfs` / `well_source_radius` say; `config` = follow those settings. Every fed cell is a well perforation in embedded mode, so feeding the whole fracture made one well carry up to 924 perforations (model5: linear setup 129 s against 1.4 s, same answer). Known limit: on a tiny propagating seed (model2, 0.28 m² split into 690 cells) the ring with a 1e-4 connection factor fails the first Newton step after binding; `embedded_perf_wi_mode=estimate` gets through but is slower and slightly worse on a large fixed fracture. |
| `embedded_wall_conduction` / `embedded_thermal_distance` | embedded flow only: heat conduction between each fracture cell and its reservoir cell, as geometric half transmissibilities (faces·area over half the aperture on the fracture side, faces·area over the leak-off distance on the rock side, or over `embedded_thermal_distance` m when given). Default on. Without it a fracture cell exchanges heat only by advection, and on a small seed fed from the ring the energy residual of the weakly flowing cells never converges. |
