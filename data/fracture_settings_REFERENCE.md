...
| `propagate_on_stable_contact` | `fractureparam.solver` | `false` | Diagnostic only — withholds propagation until the contact set matches the previous coupling round. On caked decks the set changes every round, so the seed never grows (BHP → 1000+ bar). Do not use for production. |
...
| `accept_unconverged_growth` | **top-level `solver`** | `false` | ⚠️ After the `max_growth_iterations` budget is exhausted, ACCEPT the timestep with the fracture coupling unconverged (logged as a WARNING each time) instead of letting the outer Newton chop dt. This is exactly PostSolve's per-step behavior, applied only where the coupling refuses to settle (contact-bistable establishment on filter-cake decks). Bounded compute and guaranteed progress, but the fracture state is lagged on accepted steps — **used alone it over-grows the fracture on caked decks** (35 000 m² vs PostSolve's 9 800 on the model2 10-day deck); combine with `coupling_state_relaxation≈0.5`. |
