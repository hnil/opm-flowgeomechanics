...
ExtendParam
getExtendParam(std::string filename)
{
...
extendDimens(const GridSize& grid_size, ExtendParam& extend_param, Opm::Deck& deck)
{
    ...
    if (deck.hasKeyword<ParserKeywords::DIMENS>()) {
        ...
        {
            DeckItem& eqldims = const_cast<DeckItem&>(
                deck[ParserKeywords::EQLDIMS::keywordName].back().getRecord(0).getItem("NTEQUL"));
            std::vector<int>& data = eqldims.getData<int>();
            std::vector<value::status>& value_status
                = const_cast<std::vector<value::status>&>(eqldims.getValueStatus());
...
void
extendRegions(DeckSection& regions,
              const GridSize grid_size,
              const ExtendParam& extend_param,
              const std::vector<int>& actnum_old)
{
    ...
    if (regions.hasKeyword<Opm::ParserKeywords::EQLNUM>()) {
        ...
        // std::vector<int>& eqldims =
        // const_cast<std::vector<int>&>(deck.get<Opm::ParserKeywords::EQLDIMS>().back().getIntData());
    } else {
...
void
extendSolution(DeckSection& solution, const ExtendParam& /*extend_param*/)
{
...
manipulate_deck(const char* deck_file, std::ostream& os)
{
    ...
    for (const auto& records : deck) {
        ...
        int nz_upper = extend_param.nz_upper;
        // DeckView
        if ((records.name() != std::string("EQUALS")) && (records.name() != std::string("MULTIPLY"))
            && (records.name() != std::string("ADD")) && (records.name() != std::string("COPY"))) {
