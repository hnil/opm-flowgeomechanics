  ...
  FractureMechanicsPreconditioner::FractureMechanicsPreconditioner(const Opm::SystemMatrix& S, Opm::PropertyTree prm)
        ...
        , prm_(prm)          
    {
      ...
      if(!diag_flow_){
          ...
          flow_solver_ = std::make_unique< FlowSolverType>(*flowop_, prm_.get_child("flow_solver"), std::function<Vector()>(), 1);
      }
    }   

void FractureMechanicsPreconditioner::apply(Opm::VectorHP& v, const Opm::VectorHP& d)
    {
