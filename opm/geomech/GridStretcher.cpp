...
template <typename T>
vector<T>
extract_elements(const vector<T>& vec, const vector<size_t>& ixs)
// ----------------------------------------------------------------------------
{
    vector<T> result;
...
// ----------------------------------------------------------------------------
vector<double>
pick_nodes(const vector<size_t>& ixs, const vector<double>& coords2D)
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
vector<double>
pickCoords2D(const vector<CoordType>& coords3D)
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
size_t
find_coord_in(const CoordType& c, const vector<CoordType>& cvec)
// ----------------------------------------------------------------------------
{
...
{
vector<size_t>
GridStretcher::boundary_node_indices(const Grid& grid)
{
...
// ----------------------------------------------------------------------------
vector<size_t>
GridStretcher::boundary_node_indices_old(const Grid& grid)
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
vector<size_t>
GridStretcher::complement_of(const vector<size_t>& vec, const size_t N)
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
vector<CoordType>
GridStretcher::node_coordinates(const Grid& g)
// ----------------------------------------------------------------------------
{
...
// ----------------------------------------------------------------------------
BoundaryNormals
GridStretcher::boundary_normals(const Grid& grid,
...
// ----------------------------------------------------------------------------
{
    ...
    for (size_t bix : bcindices) {
        ...
        const auto tangent = nodecoords[bnindices[get<1>(entry)]] - nodecoords[bnindices[get<2>(entry)]];
    ...
    transform(result.bnode_normals.begin(),
...
// ----------------------------------------------------------------------------
vector<double>
GridStretcher::interior_parametrization(const vector<size_t>& bindices,
...
GridStretcher::compute_cell_2_bindices_mapping(const Grid& grid, const vector<size_t>& bindices)
// ----------------------------------------------------------------------------
{
    CellBnodeMap result;
...
// ----------------------------------------------------------------------------
void
GridStretcher::expandBoundaryCells(const vector<double>& amounts)
// ----------------------------------------------------------------------------
{
    vector<double> distance = computeBoundaryNodeDisplacements(amounts, boundary_normals_.bnode_normals);
...
// ----------------------------------------------------------------------------
void
GridStretcher::updateAfterBoundaryChange(const vector<CoordType>& new_bcoords)
// ----------------------------------------------------------------------------
{
    ...
    for (size_t iix : iindices_)
        for (size_t bix = 0; bix != bnindices_.size(); ++bix)
            ncoords[iix] += ncoords[bnindices_[bix]] * (*ipiter++);
...
// ----------------------------------------------------------------------------
void
GridStretcher::applyBoundaryNodeDisplacements(const vector<CoordType>& disp)
// ----------------------------------------------------------------------------
{
