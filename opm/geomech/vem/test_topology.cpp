...
main(int varnum, char** vararg)
{
    ...
    const int total_num_cellfaces = accumulate(num_cell_faces.begin(), num_cell_faces.end(), 0);
    ...
    // read face corners
    const int total_num_face_corners = accumulate(num_face_corners.begin(), num_face_corners.end(), 0);
